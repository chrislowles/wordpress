...
class ChrisLowles_Redirects {
    ...
    public function render_meta_box($post) {
        ...
        <table class="form-table">
            <tr>
                ...
                <td><input type="text" name="redirect_path" value="<?php echo esc_attr($path); ?>" class="widefat" placeholder="example-path">
                <p class="description">Redirects from: <?php echo home_url('/'); ?><strong><?php echo esc_html($path); ?></strong></p></td>
            </tr>
            <tr>
    ...
    // Admin Columns & Search Logic (Abbreviated for bloat reduction - keeping core logic)
    public function custom_columns($columns) {
        return ['cb' => $columns['cb'], 'title' => 'Title', 'redirect_path' => 'Path', 'redirect_url' => 'Destination', 'redirect_description' => 'Description'];
    }
    ...
    public function sortable_columns($c) { $c['redirect_path'] = 'redirect_path'; $c['redirect_url'] = 'redirect_url'; return $c; }
    ...
    public function search_join($join) {
        global $wpdb, $pagenow;
        if (is_admin() && $pagenow === 'edit.php' && isset($_GET['post_type']) && $_GET['post_type'] === 'redirect' && !empty($_GET['s'])) {
    ...
    public function search_where($where) {
        ...
        if (is_admin() && $pagenow === 'edit.php' && isset($_GET['post_type']) && $_GET['post_type'] === 'redirect' && !empty($_GET['s'])) {
            ...
            $where = preg_replace("/\(\s*{$wpdb->posts}.post_title\s+LIKE\s*(\'[^\']+\')\s*\)/", "({$wpdb->posts}.post_title LIKE $1) OR ({$wpdb->postmeta}.meta_key IN ('_redirect_path','_redirect_url','_redirect_description') AND {$wpdb->postmeta}.meta_value LIKE '%{$s}%')", $where);
        }
        return $where;
    }

    public function search_distinct($distinct) { return (is_admin() && isset($_GET['post_type']) && $_GET['post_type'] === 'redirect') ? "DISTINCT" : $distinct; }
    ...
    public function handle_redirects() {
        ...
        $args = [
            'post_type' => 'redirect', 'post_status' => 'publish', 'posts_per_page' => 1,
            'meta_query' => [['key' => '_redirect_path', 'value' => $path, 'compare' => '=']],
            'no_found_rows' => true
        ];
