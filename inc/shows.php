...
class ChrisLowles_Shows {
    ...
    public function filter_author_archive_by_type( \WP_Query $query ): void {
        if ( is_admin() || ! $query->is_main_query() || ! $query->is_author() ) return;
        if ( isset( $_GET['post_type'] ) && $_GET['post_type'] === 'show' ) {
    ...
    public function __construct() {
        ...
        add_action( 'pre_get_posts', [ $this, 'filter_author_archive_by_type' ] );
    }
    ...
    public function date_enforcement_notices(): void {
        ...
        if ( ! empty( $_GET['show_date_error'] ) ) {
            echo '<div class="notice notice-error nagging"><p>You need to explicitly set a publish date. Open the <strong>Publish</strong> date picker, choose a date (past or future), click <strong>OK</strong>, then save.</p></div>';
            return;
        }
