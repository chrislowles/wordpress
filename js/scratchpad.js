jQuery(document).ready(function($) {
	var $textarea = $('#scratchpad-content');
	var $overlay = $('#scratchpad-lock-overlay');
	var $ownerName = $('#lock-owner-name');
	var $status = $('#scratchpad-status');
	var isEditing = false;
	...
	$(document).on('heartbeat-send', function(e, data) {
		// Send our status
		data.scratchpad_check = true;
		data.scratchpad_is_editing = isEditing;
	});

	// 3. Hook into Heartbeat Receive
	// This runs when the server replies
	$(document).on('heartbeat-tick', function(e, data) {
		if (!data.scratchpad_status) {
			return;
		}

		if (data.scratchpad_status === 'locked') {
			// LOCK IT DOWN
			$textarea.prop('disabled', true);
			$overlay.removeClass('hidden');
			$ownerName.text(data.scratchpad_owner);
			
			// Update content if provided (live-ish sync)
			if (data.scratchpad_content && $textarea.val() !== data.scratchpad_content) {
				$textarea.val(data.scratchpad_content);
			}
		} else {
	...
	// 5. Save Button Logic
	$('#scratchpad-save').on('click', function() {
		var content = $textarea.val();
		var $btn = $(this);
		
		$btn.prop('disabled', true).text('Saving...');
		
		$.post(scratchpadSettings.ajax_url, {
			action: 'save_scratchpad',
			nonce: scratchpadSettings.nonce,
			content: content
